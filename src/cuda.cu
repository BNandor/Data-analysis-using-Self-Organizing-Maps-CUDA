...
__global__ void dev_getDistances(double* sample,double* map,int dim, double* dist){
...
__device__	double normalNeighbourCoefficient(int i1, int j1, int i2, int j2, double radius)
	{
		double euclideanDist = euclideanDistance(i1, j1, i2, j2);
		return normal_pdf(euclideanDist, 0, radius);
	}

__global__ void dev_updateNeighbours(int closest_i, int closest_j, double learningRate, double neighbourRadius, double* sample, int dim, int mapWidth, int mapHeight, double* map)
	{
		int offset = blockIdx.x;
		double* protoype = map + offset*dim;
		double* diff = difference(sample,protoype,dim);
		double ncof = normalNeighbourCoefficient(closest_i, closest_j, offset/mapWidth, offset%mapWidth, neighbourRadius);
		for(int c=0;c<dim;c++){
