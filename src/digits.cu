...
int main(int argc, const char *argv[])
{
	...
	if (!options.count("test"))
	{
		...
		std::ofstream output;
		std::string outputName = options.count("outputPath") > 0 ? options["outputPath"] : OUTPUT_SOM_FILENAME;
...
SOM::SelfOrganizingMap trainSingleSOM(io::argumentOptions options) {
		...
		if (conf.animation)
		{
			som.train(conf.maxT, [&](int T, int maxT, SOM::SelfOrganizingMap *map) {
				...
				{
					std::ofstream file;
					file.open(conf.animationPath + (std::to_string(T / (maxT / conf.frameCount)) + ".som").c_str());
...
double sampleClassificationAccuracy(SOM::configuration conf,digitSet& splitTrain, digitSet& splitTest){
	...
	double accuracy = std::count_if(splitTest.getDigits().begin(), splitTest.getDigits().end(), [&](auto &sample) {
																										return SOM::classify(maps, sample);
																										}
...
std::vector<double> classificationAccuracies(io::argumentOptions options) {
		...
		for(int i=0;i<k;++i){
			...
			accuracies.push_back(sampleClassificationAccuracy(conf,splitData.first,splitData.second));
		}
		return accuracies;
}