...
namespace io {
    ...
    namespace SOM {
        ::SOM::configuration parse_SOM_configuration(argumentOptions options) {
            ...
            conf.mapW = options.count("mapw") > 0 ? std::atoi(options["mapw"].c_str()) : DEFAULT_MAP_WIDTH;
            conf.mapH = options.count("maph") > 0 ? std::atoi(options["maph"].c_str()) : DEFAULT_MAP_HEIGHT;
            ...
            conf.frameCount = options.count("framecount") > 0 ? std::atoi(options["framecount"].c_str()) : ANIMATION_DEFAULT_FRAMECOUNT;

            conf.classification = options.count("test") > 0;
            conf.classCount = options.count("classCount") > 0 ? std::atoi(options["classCount"].c_str()) : CLASSIFICATION_DEFAULT_CLASS_COUNT;
            return conf;
        }
